...
To refresh the live website manually, open the repository’s **Actions** tab, select **Sync inventory**, choose **Run workflow**, and confirm. The same workflow also runs automatically at 17 minutes past every hour in the `Europe/Istanbul` timezone. It runs the same read-only synchronizer, publishes the latest catalogue and sync-status files, and GitHub Pages publishes the new snapshot automatically. The workflow summary reports added, updated, unchanged, missing-list, missing-image, skipped-photo-folder, and folder-error counts. The Sales Mode dashboard keeps the five most recent sync attempts visible, including the latest failure message. A failed sync does not replace the previous catalogue.
